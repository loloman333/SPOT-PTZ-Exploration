...
vscode_dir="/home/appuser/.vscode-server/bin/${vscode_commit_sha}"
echo "will attempt to download VS Code Server version = '${vscode_commit_sha}'"
# Download VS Code Server tarball to tmp directory.
echo curl -L "https://update.code.visualstudio.com/commit:${vscode_commit_sha}/server-linux-${ARCH}/stable" -o "/tmp/${archive}"
...
tar --no-same-owner -xzv --strip-components=1 -C "$vscode_dir" -f "/tmp/${archive}" > /dev/null 

# Install extensions from .devcontainer.json
# Exclude comments lines to make file a valid JSON
extensions_json=$(cat "$devcontainer_path" | grep -vE "\s*//")
if echo $extensions_json | jq -e '.customizations.vscode.extensions';
then
    extensions_list=($(echo $extensions_json | jq -r '.customizations.vscode.extensions[]'))
else
    echo "customizations.vscode.extensions attribute does not exist. Not installing any extensions."
fi
if [ -n "$extensions_list" ]; 
then
