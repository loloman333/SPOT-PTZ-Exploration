...
# You should run this script from the misc/docker/bases directory! So not ./misc/docker/bases/run_docker_ros2.sh!
...
if [[ $( docker ps -a -f name=test_spot_ros2_base | wc -l ) -eq 2 ]];
then
    ...
    select yn in "Restart" "Remove"; do
        case $yn in
            Restart )
                ...
                docker restart test_spot_ros2_base;
                break;;
            Remove )
                echo "Stopping it and deleting it... You should simply run this script again to start it.";
                docker stop test_spot_ros2_base;
                docker rm test_spot_ros2_base;
                break;;
        esac
    done
else
    ...
    docker run \
        ...
        --detach \
        --privileged \
        --name test_spot_ros2_base \
        spot-ros2:ros2
    docker exec -u appuser test_spot_ros2_base bash -c 'python3 /home/appuser/ros2_ws/test_required_packages.py'
fi