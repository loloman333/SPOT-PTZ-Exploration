...
check_dependencies() {
    ...
    ################## TODO: put these commands back if CUDA is needed ##################
...
then
    ...
    ################## TODO: use either the CUDA or the non-CUDA version ##################
    # run container with CUDA (makes sense even for non-cuda image because rviz can sometimes run faster with GPU)
    docker run \
        --env DISPLAY=${DISPLAY} \
        --volume /tmp/.X11-unix:/tmp/.X11-unix \
        --volume ${my_packages_folder}:/home/appuser/ros2_ws/src/my_packages \
        --volume ${vscode_folder}:/home/appuser/ros2_ws/.vscode \
        --volume ${data_folder}:/home/appuser/ros2_ws/data \
        --volume ../.bash_history:/home/appuser/.bash_history \
        --network host \
        --interactive \
        --tty \
        --detach \
    ...
    # run container without CUDA (for systems without NVIDIA GPU)
    # docker run \
    #     --env DISPLAY=${DISPLAY} \
    #     --volume /tmp/.X11-unix:/tmp/.X11-unix \
    #     --volume ${my_packages_folder}:/home/appuser/ros2_ws/src/my_packages \
    ...
    echo "🔧 Forcing permissions INSIDE the container..."
    docker exec -u 0 $container_name chmod 666 /dev/dri/renderD128
    docker exec -u 0 $container_name chmod 666 /dev/dri/card* || true

    docker exec -u appuser $container_name bash -c 'python3 /home/appuser/ros2_ws/test_required_packages.py'
fi
